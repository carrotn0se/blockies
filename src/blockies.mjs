...
function seedrand(seed, target) {
	...
	for(let i = 0; i < seed.length; i++) {
		randseeds[target][i%4] = ((randseeds[target][i%4] << 5) - randseeds[target][i%4]) + seed.charCodeAt(i);
	}
}
...
function rand(target) {
	...
	randseeds[target][3] = (randseeds[target][3] ^ (randseeds[target][3] >> 19) ^ t ^ (t >> 8));
...
function createColor(target) {	
	...
	const l = ((rand(target) + rand(target) + rand(target) + rand(target)) * 25) + '%';
